...
function Dykstra_lamCC_TFA(A::SparseMatrixCSC{Float64,Int64},AppTol::Float64=1e-3,ConTol::Float64=1e-3,
        ...
        while iter < maxits
                ...
                X = E[:,:]+D[:,:]
                ConCheck, lastConCheck, lastTriCheck, objective, Ratio, SlackMax, TotalSlack = report_progress(A,D,X,W,E,F,P,Q,next_corrections,filename,vecChange,iter,ConTol,lam,lastConCheck,lastTriCheck,BtDual,TriTime,gam,SlackMax,TotalSlack)

                # We stop if the constraints are sufficiently satisfied
                # and the approximation ratio is small enough

                if Ratio < AppTol && ConCheck && Ratio > .5 && iter > 5000
                  break
                end
...
function report_progress(A::SparseMatrixCSC{Float64,Int64},D::Matrix{Float64},X::Matrix{Float64},W::Matrix{Float64},E::Matrix{Float64},F::Matrix{Float64},P::Matrix{Float64},Q::Matrix{Float64},current_corrections::Vector{Tuple{Int64,Float64}},
    filename::String,xChange::Float64,iter::Int64,ConTol::Float64,lam::Float64,lastConCheck::Float64,lastTriCheck::Float64,BtDual::Float64,triTime::Float64,gam::Float64,SlackMax::Float64,TotalSlack::Float64)
        ...
        #println("$iter:\t Dual = $DuRound, Primal = $PriRound, gap = $gapround, ConSat: $lastConCheck, 3Loop: $time, LPobj: $ob, nnz(y) = $nnz, sparsity = $ysparsity")

        open(filename, "a") do f
          write(f,"$iter:\t Dual = $DuRound, Primal = $PriRound, gap = $gapround, ConSat: $lastConCheck, 3Loop: $time, LPobj: $ob, nnz(y) = $nnz\n")
        end
