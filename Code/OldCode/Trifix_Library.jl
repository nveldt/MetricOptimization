...
function Dykstra_TriangleFix01bounds(A::SparseMatrixCSC{Float64,Int64},D::Matrix{Int8},E::Matrix{Float64},F::Matrix{Float64},W::Matrix{Float64},Etol::Float64,TriTol::Float64,lam::Float64,filename::String,gam::Float64)
n = size(A,1)
@show filename
  open(filename, "w") do f
          write(f, "Writing output from calling the Triangle Fixing Algorithm\n")
...
# Got through and check whether things are between 0 and 1. If not, make them so
for i = 1:n-1
  for j = i+1:n
    if float(D[j,i]) + E[j,i]  > 1
      E[j,i] = 1 - float(D[j,i])
    end
    if  E[j,i] + float(D[j,i]) < 0
      E[j,i] = -float(D[j,i])
    end
  end
end
...
while true && iter < maxits
    ...
    # Start again with the triangle inequality constraints
    tic()
    Kcounter = HLWB_TripleLoop!(D,E,W,CE,CF,Kcounter)
    lasttime = toq()
