...
function TripleLoop2!(D::Matrix{Int8},E::Matrix{Float64},W::Matrix{Float64},now_corrections::Vector{Tuple{Int64,Int64,Int64,Float64}},next_corrections::Vector{Tuple{Int64,Int64,Int64,Float64}})
  n = size(D,1)
  epsi = 1e-8
  ...
  @inbounds for i = 1:n-2
    ...
    if i == nextTriplet[1] && j == nextTriplet[2] && k == nextTriplet[3]
      ...
      # This is a slight error: if there is a correction, we need to actually
      # update variable E[j,i] and the others, not just update the temporary
      # variable Eij before checking if mu > epsi
      #
      # However, even when you fix it, experimentally I see little to no change
      # Still, don't use this version of the code
...
while true && iter < maxits
    ...
    tic()
    #TripleLoop2!(D,E,W,current_corrections,next_corrections)
    FirstTripleLoop!(D,E,W,current_corrections) # try just completely ignoring the corrections on these, I'm pretty sure they are not needed.
    lasttime = toq()
